...
from B_Model.Autoformer.models.Transformer import Model as Transformer
...
class Model(AbstactModel):
    ...
    def __get_input__(self, x:np.float32_3d[ax.sample, ax.time, ax.feature]):
        ...
        if (self.CTX["EMBEDDING_SIZE"] > 0):
            ...
            x_mark_dec = torch.tensor(x[:, self.CTX["HISTORY"]:, -self.CTX["EMBEDDING_SIZE"]:])
        else:
            x_mark_enc = torch.zeros(len(x), self.CTX["HISTORY"], 0)
            x_mark_dec = torch.zeros(len(x), self.CTX["LOOK_AHEAD"], 0)
