...
class Model(AbstactModel):
    ...
    def __init__(self, CTX:dict):
        ...
        for i in range(self.CTX["LSTM_LAYERS"]-1):
            z = LSTM(self.CTX["LSTM_UNITS"], return_sequences=True, dropout=self.dropout)(z)
            
        if (self.CTX["SLIDING_WINDOW"]):
            z = LSTM(self.CTX["LSTM_UNITS"], return_sequences=False, dropout=self.dropout)(z)
            z = Dense(CTX["OUTPUT_LEN"], activation="linear")(z)
        else:
            z = LSTM(1, return_sequences=True, dropout=self.dropout)(z)
